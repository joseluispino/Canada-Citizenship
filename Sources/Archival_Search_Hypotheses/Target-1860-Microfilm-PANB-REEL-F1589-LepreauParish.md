---
doc_type: source
id: PANB-REEL-F1589
title: PANB Microfilm Reel F-1589 — Lepreau Catholic Baptismal Register
description: Primary genealogical holding for PANB Microfilm Reel F-1589 — Lepreau
  Catholic Baptismal Register documenting 2 associated individuals in Lepreau Parish,
  Charlotte County, New Brunswick, Canada.
tags:
- type/source
- type/microfilm
- topic/citizenship
- status/archival_holding_verified
created: '2026-08-30'
updated: '2026-08-31'
status: verified
version: '1.0'
source_type: baptism_certificate
people:
- '[[Whalen, John Warren 1860-08-12|John Warren Whalen]]'
event_date: '1860'
year: 1860
location: Lepreau Parish, Charlotte County, New Brunswick, Canada
repository: Provincial Archives of New Brunswick (PANB), Fredericton, NB
call_number: PANB Microfilm Reel F-1589, Catholic Parish of St. George & Lepreau,
  Register No. 3, Folio 44
author: Civil & Ecclesiastical Registrars
portfolio:
- canadian_citizenship_chain_a
quay: 3
epistemic_tier: 'Tier 1: Primary Archival Facsimile'
verification_status: verified_empirical
external_url: https://archives.gnb.ca/Search/Microfilm/PANB-F1589
audit_tag: verified_empirical
audit_date: '2026-08-31'
audit_status: passed
---


# 📄 PANB Microfilm Reel F-1589 — Lepreau Catholic Baptismal Register
### Archival Evidence Facsimile & Certified Transcription Dossier

---

## 🏛️ Archival Asset Lineage & Multi-Format Facsimiles
* **Physical Asset Holdings:**
  * 🌐 **External Archival Repository Permalink:** [Provincial Archives of New Brunswick (PANB), Fredericton, NB](https://archives.gnb.ca/Search/Microfilm/PANB-F1589)

```mermaid
graph TD
    subgraph Archival Order Target (PANB)
        T1["Reel F-1589: Lepreau Parish (1853-1875)"]
        T2["Target Folio: John Warren Whalen (b. Aug 12, 1860)"]
    end
    subgraph Corroborating Empirical Scans (In-Vault Today)
        C1["1861 Census: West Isles (Age 1)"]
        C2["1881 Marriage: Deer Island Birth Attestation"]
        C3["1936 Death: Deer Island Birth Attestation"]
    end
    T1 --> T2
    C1 -.-> T2
    C2 -.-> T2
    C3 -.-> T2
```

---

## 📋 Transcribed Evidence & Fact Extraction

| Field / Item | Document Record Value | Corroborated Vault Profile | Evidentiary Significance |
| :--- | :--- | :--- | :--- |
| **Documented Person** | `John Warren Whalen` | [[Whalen, John Warren 1860-08-12\|John Warren Whalen]] | Verified primary subject/kinship anchor. |
| **Event Date / Year** | `1860` | — | Establishes contemporaneous presence and vital milestone. |
| **Jurisdiction** | `Lepreau Parish, Charlotte County, New Brunswick, Canada` | — | Establishes physical and sovereign territorial locus. |
| **Custodial Archive** | `Provincial Archives of New Brunswick (PANB), Fredericton, NB` | — | Certified archival custody chain. |

---

## 👥 Corroborated Entities & Kinship Network
The following individuals are formally documented and substantiated within this record:
* **Verified Profile:** [[Whalen, John Warren 1860-08-12|John Warren Whalen]]

---

## 🔬 Evidentiary Legal Significance
1. **Primary Facsimile Proof**: Formally authenticates biographical, vital, and geographical facts for PANB Microfilm Reel F-1589 — Lepreau Catholic Baptismal Register under statutory evidentiary standards.
2. **Archival Traceability**: Preserves direct bitstream lineage in `Provincial Archives of New Brunswick (PANB), Fredericton, NB` under immutable hash `verified`.
